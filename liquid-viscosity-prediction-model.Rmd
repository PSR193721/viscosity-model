...
<p>Now load in the data that we want to work with. We will use ethane to test our regression model, so we'll load it now.</p>
...
## Model Setup
<p>Now we need to create the data set that we'll us to determine our regression coefficients. The goal is to reproduce the coefficients published by <em>Van Velzen et al</em>. We need to extract the published values for $NE$ and $T_0$ from the appendices. Then, those data will be used to run a few different polynomial regression models on the data. The regression models will all use a simply monomial basis in a Vandermonde matrix.</p>
...
df_raw = data.frame(NE_exp, T0, B_exp)
...
```{r}
fit_quadratic_T0 = lm(T0 ~ N + N2, data = df_raw)
summary(fit_quadratic_T0)

fit_cubic_T0 = lm(T0 ~ N + N2 + N3, data = df_raw)
summary(fit_cubic_T0)

fit_quartic_T0 = lm(T0 ~ N + N2 + N3 + N4, data = df_raw)
...
```{r}
fit_quadratic_B = lm(B_exp ~ N + N2, data = df_raw)
summary(fit_quadratic_B)

fit_cubic_B = lm(B_exp ~ N + N2 + N3, data = df_raw)
summary(fit_cubic_B)

fit_quartic_B = lm(B_exp ~ N + N2 + N3 + N4, data = df_raw)
...
B_calc <- function(N) {
  B_approx = 24.79 + 66.885*N - 1.3173*N^2 - 0.00377*N^3
  return (B_approx)
}
...
T0_ethane = round(T0_calc(T_coeffs, 2), 2)
B_ethane = round(B_calc(2), 2)

viscosity_calc(T0_ethane, B_ethane, -170)


ethane_tbl = ethane_tbl %>% 
  mutate(CalcTest = viscosity_calc(T0_ethane, B_ethane, ethane_tbl$Temp))
...
```{r}
T0_approx = 1.009 + betas[1]*df_working$Temp + betas[2]*df_working$Temp + betas[3]*df_working$Temp
```
